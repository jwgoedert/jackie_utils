...
// Configuration
const ROOT_DIR = "/Volumes/T7 Shield/JACKIESUMELL.COM/NV Abolitionist's Sanctuary 2021-2024"; // Replace this with your actual top-level path
const MAX_DEPTH = 4;
// Fix: Use a more comprehensive approach to detect apostrophes
const FIND_PATTERN = /\bAbolitionist\b/; 
const REPLACE_PATTERN = "The Abolitionist"; // U+2019
...
async function appendTheToProjectName(dir, depth = 0) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (FIND_PATTERN.test(entry.name)) {
        // Reset the regex for the next test
        FIND_PATTERN.lastIndex = 0;
        ...
        if (isYearPrefixed || isInYearDir) {
          ...
          const newName = entry.name.replace(FIND_PATTERN, REPLACE_PATTERN);
...
appendTheToProjectName(ROOT_DIR)
  .then(() => {
    printSummary();
